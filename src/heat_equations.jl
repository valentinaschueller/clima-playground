...
function coupled_heat_equations(;
    ...
    elseif plot_unstable_range
        ...
        n_zs_atm =
            ...
            Int.(
                round.(
                    (physical_values[:h_atm] - physical_values[:z_0numA]) ./
                    reverse(delta_z)
                )
            ) : nothing
        n_ts_atm =
            atm ? Int.(round.(physical_values[:delta_t_min] ./ reverse(delta_t))) : nothing
        n_zs_oce =
            oce ?
            Int.(
                round.(
                    (physical_values[:h_oce] - physical_values[:z_0numO]) ./
                    reverse(delta_z)
                )
            ) : nothing
        n_ts_oce =
            oce ? Int.(round.(physical_values[:delta_t_min] ./ reverse(delta_t))) : nothing
        ...
        for a_i in a_is
            ...
            if atm
                unstable_matrix_atm, theoretical_vals_matrix_atm =
                ...
                theoretical_vals_matrix_atm =
                    reverse(theoretical_vals_matrix_atm, dims = (1, 2))
                plot_delta_z_delta_t(
                    unstable_matrix_atm,
                    theoretical_vals_matrix_atm,
                    delta_z,
                    delta_t,
            ...
            elseif oce
                unstable_matrix_oce, theoretical_vals_matrix_oce =
                ...
                theoretical_vals_matrix_oce =
                    reverse(theoretical_vals_matrix_oce, dims = (1, 2))
                plot_delta_z_delta_t(
                    unstable_matrix_oce,
                    theoretical_vals_matrix_oce,
                    delta_z,
