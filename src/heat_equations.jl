    ...
    for t = tspan[begin]:Δt_cpl:(tspan[end]-Δt_cpl)
        ...
        for iter = 1:iterations
            ...
            if !(atm_stable && oce_stable)
                ...
                ρ_A = atm_stable ? NaN : Inf
                ρ_O = oce_stable ? NaN : Inf
                return ρ_A, ρ_O
            end
        ...
        if iterations > 1
            ρ_A, ρ_O = compute_ρ_numerical(atmos_vals_list, ocean_vals_list)
        end
    end
...
function coupled_heat_equations(;
    ...
    cs, ρ_A, ρ_O = run_simulation(physical_values, iterations=iterations, parallel=parallel)
    return cs, ρ_A, ρ_O
end;
