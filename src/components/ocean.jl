...
function ocean_init(
    ...
    cache = (
        params = parameters,
        area_fraction = 1 - parameters.a_i,
        T_air = Float64(0),
    )
    ...
    integrator = SciMLBase.init(problem, stepping.odesolver, dt = Δt, saveat = saveat, adaptive = false)
...
Interfacer.get_field(sim::HeatEquationOcean, ::Val{:area_fraction}) = sim.integrator.p.area_fraction
...
FluxCalculator.update_turbulent_fluxes!(sim::HeatEquationOcean, fields::NamedTuple) = nothing
