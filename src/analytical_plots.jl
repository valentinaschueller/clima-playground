...
function plot_obukhov_C_dependencies(C_variable_name)
    ...
    for (i, a_i) in enumerate(a_is)
        for (j, pos_var) in enumerate(pos_vars)
            physical_values[:L_AO] = (C_variable_name == "C_AO") ? pos_var : physical_values[:L_AO]
            physical_values[:L_AI] = (C_variable_name == "C_AI") ? pos_var : physical_values[:L_AI]
            physical_values = (C_variable_name == "C_AO") ? update_C_AO(physical_values) : update_physical_values(a_i, physical_values)
            C_pos[i, j] = (C_variable_name == "C_AO") ? physical_values[:C_AO] : physical_values[:C_AI]
        end
        for (j, neg_var) in enumerate(neg_vars)
            physical_values[:L_AO] = (C_variable_name == "C_AO") ? neg_var : physical_values[:L_AO]
            physical_values[:L_AI] = (C_variable_name == "C_AI") ? neg_var : physical_values[:L_AI]
            physical_values = (C_variable_name == "C_AO") ? update_C_AO(physical_values) : update_physical_values(a_i, physical_values)
            C_neg[i, j] = (C_variable_name == "C_AO") ? physical_values[:C_AO] : physical_values[:C_AI]
        end
    end
    ...
    if C_variable_name == "C_AO"
        ...
        plot(pos_vars, vec(C_pos), xlabel=L"$L^A_O$", ylabel=L"$C^A_O$", label="", color=:black)
...
function compute_rho_with_nu_and_omega_variable(params)
    real_part_o = sqrt((sqrt(params[:nu]^2 + params[:omega]^2) + params[:nu]) / (2 * params[:alpha_o]))
    imag_part_o = im * sign(params[:omega]) * sqrt((sqrt(params[:nu]^2 + params[:omega]^2) - params[:nu]) / (2 * params[:alpha_o]))
    sigma_o = real_part_o + imag_part_o
    real_part_a = sqrt((sqrt(params[:nu]^2 + params[:omega]^2) + params[:nu]) / (2 * params[:alpha_a]))
    imag_part_a = im * sign(params[:omega]) * sqrt((sqrt(params[:nu]^2 + params[:omega]^2) - params[:nu]) / (2 * params[:alpha_a]))
    sigma_a = real_part_a + imag_part_a
    eta_AO = params[:C_AO] * abs(params[:u_atm] - params[:u_oce]) * params[:rho_atm] * params[:c_atm]
    ...
    return abs((1 - params[:a_i])^2 * eta_AO^2 / ((params[:k_oce] * sigma_o * (1 / tanh(sigma_o * (params[:h_oce] - params[:z_0numO]))) + (1 - params[:a_i]) * eta_AO + params[:a_i] * eta_OI) * (params[:k_atm] * sigma_a * (1 / tanh(sigma_a * (params[:h_atm] - params[:z_0numA]))) + (1 - params[:a_i]) * eta_AO + params[:a_i] * eta_AI)))
end