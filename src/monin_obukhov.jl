...
function compute_C_H_AI(p::SimulationParameters; κ=0.4, L_AI=50.0, z_rTAI=1e-3)
    momentum_contribution = log(p.z_A0 / z_ruAI(p.a_I)) - Ψ_A_u(p.z_A0 / L_AI)
    heat_contribution = log(p.z_A0 / z_rTAI) - Ψ_A_T(p.z_A0 / L_AI)
...
function compute_C_H_AO(p::SimulationParameters; α_eos=1.8e-4, κ=0.4, L_AO=50.0, z_ruAO=2e-4, z_rTAO=2e-4)
    λ_u = sqrt(p.ρ_A / p.ρ_O)
    λ_T = λ_u * p.c_A / p.c_O
    μ = p.ν_O / p.ν_A
    L_OA = λ_u^2 / (p.T_A_ini * α_eos * λ_T)
    momentum_atm = log(p.z_A0 / z_ruAO) - Ψ_A_u(p.z_A0 / L_AO)
    momentum_oce = log(λ_u * p.z_O0 / (z_ruAO * μ)) - Ψ_O_u(p.z_O0 / L_OA)
    momentum_contribution = momentum_atm + λ_u * momentum_oce

    heat_atm = log(p.z_A0 / z_rTAO) - Ψ_A_T(p.z_A0 / L_AO)
    heat_oce = log(λ_T * p.z_O0 / (z_rTAO * μ)) - Ψ_O_T(p.z_O0 / L_OA)
